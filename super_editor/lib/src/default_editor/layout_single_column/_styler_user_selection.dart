...
class SingleColumnLayoutSelectionStyler extends SingleColumnLayoutStylePhase {
  ...
  ValueListenable<DocumentSelection?> get selection => _selection;
...
class SingleColumnLayoutSectionSelectionStyler extends SingleColumnLayoutStylePhase {
  SingleColumnLayoutSectionSelectionStyler({
    required Document document,
    required ValueListenable<DocumentSelection?> sectionSelection,
    required SelectionStyles selectionStyles,
    SelectedTextColorStrategy? selectedTextColorStrategy,
  })  : _document = document,
        _sectionSelection = sectionSelection,
        _sectionSelectionStyles = selectionStyles,
        _selectedTextColorStrategy = selectedTextColorStrategy {
    // Our styles need to be re-applied whenever the document selection changes.
    _sectionSelection.addListener(markDirty);
  }

  @override
  void dispose() {
    _sectionSelection.removeListener(markDirty);
    super.dispose();
  }

  final Document _document;
  final ValueListenable<DocumentSelection?> _sectionSelection;

  SelectionStyles _sectionSelectionStyles;
  set selectionStyles(SelectionStyles selectionStyles) {
    if (selectionStyles == _sectionSelectionStyles) {
      return;
    }
  ...
  SingleColumnLayoutViewModel style(Document document, SingleColumnLayoutViewModel viewModel) {
    editorStyleLog.info("(Re)calculating selection view model for document layout");
    editorStyleLog.fine("Applying selection to components: ${_sectionSelection.value}");
    return SingleColumnLayoutViewModel(
      padding: viewModel.padding,
      componentViewModels: [
        for (final previousViewModel in viewModel.componentViewModels) //
  ...
  SingleColumnLayoutComponentViewModel _applySelection(SingleColumnLayoutComponentViewModel viewModel) {
    final documentSelection = _sectionSelection.value;
    final node = _document.getNodeById(viewModel.nodeId)!;
    ...
    if (node is TextNode) {
      ...
      final highlightWhenEmpty = nodeSelection == null
          ? false
          : nodeSelection.highlightWhenEmpty && _sectionSelectionStyles.highlightEmptyTextBlocks;
      ...
      if (viewModel is TextComponentViewModel) {
        ...
        final textWithSelectionAttributions = textSelection != null &&
            ...
            ? (viewModel.text.copyText(0)
              ..addAttribution(
                ColorAttribution(_selectedTextColorStrategy!(
                  originalTextColor: componentTextColor,
                  selectionHighlightColor: _sectionSelectionStyles.selectionColor,
                )),
                SpanRange(textSelection.start, textSelection.end - 1),
                // The selected range might already have a color attribution. We want to override it
                // with the selected text color.
        ...
        viewModel
          ..text = textWithSelectionAttributions
          ..sectionSelection = textSelection
          ..sectionSelectionColor = _sectionSelectionStyles.selectionColor
          ..highlightWhenEmpty = highlightWhenEmpty;
      }
    }
    if (viewModel is SelectionAwareViewModelMixin) {
      viewModel
        ..sectionSelection = nodeSelection
        ..sectionSelectionColor = _sectionSelectionStyles.selectionColor;
    }
